...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("6) Mint and Bridge Zerius NFT", mint_zerius),
            Choice("7) Create NFT collection on Omnisea", create_omnisea),
            Choice("8) Mint NFT on NFTS2ME", mint_nft),
            Choice("9) Dmail send email", send_mail),
            Choice("10) Use custom routes", custom_routes),
            Choice("11) Check transaction count", "tx_checker"),
            Choice("12) Exit", "exit"),
        ],
        qmark="⚙️ ",
        pointer="✅ "
