...
def get_module():
    result = questionary.select(
        ...
        choices=[
            ...
            Choice("12) Mint ZkStars NFT", mint_zkstars),
            Choice("13) Create NFT collection on Omnisea", create_omnisea),
            Choice("14) Mint NFT on NFTS2ME", mint_nft),
            Choice("15) Dmail send email", send_mail),
            Choice("16) Create gnosis safe", create_safe),
            Choice("17) Deploy contract", deploy_contract),
            Choice("18) Use custom routes", custom_routes),
            Choice("19) Check transaction count", "tx_checker"),
            Choice("20) Exit", "exit"),
        ],
        qmark="⚙️ ",
        pointer="✅ "
